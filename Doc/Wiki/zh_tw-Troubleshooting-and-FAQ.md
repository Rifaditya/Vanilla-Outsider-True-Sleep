...
* **Answer**: True Sleep automatically detects standard machine patterns (`furnace`, `smelter`, `generator`, `crusher`, `alloy`). For custom block entities with non-standard names, add their ID to the datapack tag `#truesleep:accelerated_machines`. See [[API & Data Tags|zh_tw-API-and-Tags]].
